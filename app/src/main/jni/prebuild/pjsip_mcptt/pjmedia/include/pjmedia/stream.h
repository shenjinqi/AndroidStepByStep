...
typedef void (*on_rtcp_app_rx_cb)( pjmedia_rtcp_session *sess,
                   const void *pkt,
                   pj_size_t size);


PJ_DECL(void)
pjmedia_stream_set_rtcp_app_rx_cb( on_rtcp_app_rx_cb cb);
...
PJ_DECL(pj_status_t) pjmedia_stream_tx_start(pjmedia_stream *stream);

PJ_DECL(pj_status_t) pjmedia_stream_rx_start(pjmedia_stream *stream);
